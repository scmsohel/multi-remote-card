    ...
    this.shadowRoot.innerHTML = `
      <style>
        :host {
          ...
          --ur-dark:#292a2b;
          --ur-muted:#8f8d89;
          --ur-shadow:0 8px 18px rgba(0,0,0,.10),0 2px 5px rgba(0,0,0,.06);
        }
        * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
        .card {
          width:100%; max-width:720px; margin:auto; padding:28px 28px 30px;
          border-radius:42px;
          background:linear-gradient(145deg,#f5f3f0,#e5e1db);
          color:#292929; font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;
          box-shadow:inset 0 1px 0 rgba(255,255,255,.85),0 10px 35px rgba(0,0,0,.15);
        }
        .dot { width:15px;height:15px;margin:0 auto 22px;border-radius:50%;background:#cfcfcb; }
        .dot.blink { animation:dotBlink .45s ease-out; }
        @keyframes dotBlink { 0%{background:#cfcfcb;box-shadow:none} 35%{background:#4da3ff;box-shadow:0 0 0 7px rgba(77,163,255,.16),0 0 18px rgba(77,163,255,.75)} 100%{background:#cfcfcb;box-shadow:none} }
        ...
        .speed { position:absolute;width:78px;height:78px;border:0;border-radius:50%;background:var(--ur-white);color:#272727;font-size:25px;font-weight:600;box-shadow:var(--ur-shadow);cursor:pointer;transform:translate(-50%,-50%);transition:background .18s,box-shadow .18s; }
        .speed.active { background:var(--ur-dark);color:#fff; }
        /* Final requested pattern: 3 / 2-4 / 1-5 / 6 */
        ...
        .fan-icon { width:52px;height:52px;display:block; }
        .reverse { display:block;margin:22px auto 0;min-width:180px;height:58px;border:0;border-radius:40px;background:var(--ur-white);color:#292929;box-shadow:var(--ur-shadow);font-size:17px;font-weight:700;letter-spacing:1px;cursor:pointer; }
        .modes { display:grid;grid-template-columns:repeat(3,1fr);gap:16px;margin-top:38px; }
        .mode { min-height:65px;border:0;border-radius:35px;background:var(--ur-white);color:#292929;box-shadow:var(--ur-shadow);font-size:20px;font-weight:600;cursor:pointer; }
        .light-icon { width:31px;height:31px;vertical-align:middle; }
        .timer-title { margin-top:38px; }
        .timer { display:grid;grid-template-columns:repeat(3,1fr);gap:16px; }
        .timer button { min-height:65px;border:0;border-radius:35px;background:var(--ur-white);color:#292929;box-shadow:var(--ur-shadow);font-size:19px;font-weight:700;cursor:pointer; }
        button:active { transform:scale(.97); }
        @media(max-width:520px){
          .card{padding:22px 16px 24px;border-radius:30px}.room{min-height:56px;font-size:16px}
          .fan-area{width:min(360px,100%)}.speed{width:62px;height:62px;font-size:21px}.fan-button{width:92px;height:92px}.fan-icon{width:44px;height:44px}
          .modes,.timer{gap:10px}.mode,.timer button{min-height:58px;font-size:17px}
        }
      </style>
      ...
      <div class="card">
        ...
        <div class="fan-area">
          ...
          <button class="fan-button" id="power" aria-label="Power">
        ...
        <div class="modes">
          ...
          <button class="mode" id="light" aria-label="Light">
            <svg class="light-icon" viewBox="0 0 32 32"><path d="M16 4a8 8 0 0 0-5 14c1 1 2 2 2 4h6c0-2 1-3 2-4a8 8 0 0 0-5-14z" fill="none" stroke="#111" stroke-width="2"/><path d="M13 25h6M14 28h4" stroke="#111" stroke-width="2" stroke-linecap="round"/></svg>
          </button>
