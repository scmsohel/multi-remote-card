...
class UniversalRemoteCard extends HTMLElement {
        ...
        <button class="fan-button ${on ? 'on' : ''}" id="power">
          <svg class="fan-icon" viewBox="0 0 64 64">
            <g fill="${on ? '#fff' : '#111'}">
              ...
              <circle cx="32" cy="32" r="6" fill="${on ? '#292a2b' : '#fff'}"/>
            </g>
          </svg>
        </button>
      </div>
  ...
  render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .card{width:100%;max-width:720px;margin:auto;padding:28px;border-radius:42px;background:linear-gradient(145deg,#f5f3f0,#e5e1db);color:#292929;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;box-shadow:inset 0 1px 0 #fff,0 10px 35px #0002}
      .dot{width:15px;height:15px;margin:0 auto 22px;border-radius:50%;background:#cfcfcb}.dot.blink{animation:b .45s ease-out}
      @keyframes b{0%{background:#cfcfcb;box-shadow:none}35%{background:#4da3ff;box-shadow:0 0 0 7px #4da3ff29,0 0 18px #4da3fb}100%{background:#cfcfcb;box-shadow:none}}
      .rooms{display:${this._multiple()?'grid':'none'};grid-template-columns:1fr 1fr;padding:6px;border-radius:50px;background:#d8d6d2}
      .room{border:0;min-height:68px;border-radius:42px;background:transparent;color:#8d8a86;font-size:20px;font-weight:700;cursor:pointer}.room.active{background:#292a2b;color:#fff;box-shadow:0 5px 12px #0003}
      .title{margin:38px 0 18px;text-align:center;color:#8f8d89;font-size:18px;font-weight:700;letter-spacing:4px}
      .fan-area{position:relative;width:min(430px,100%);aspect-ratio:1;margin:auto;border-radius:50%;background:radial-gradient(circle,#fff9,#e0dcd675);border:1px solid #fff;box-shadow:inset 0 0 20px #fff8,0 15px 30px #0001}
      .speed{position:absolute;width:78px;height:78px;border:0;border-radius:50%;background:#fbfaf8;color:#272727;font-size:25px;font-weight:600;box-shadow:0 8px 18px #0002,0 2px 5px #0001;cursor:pointer;transform:translate(-50%,-50%)}.speed.active{background:#292a2b;color:#fff}
      .s1{left:20%;top:72%}.s2{left:20%;top:28%}.s3{left:50%;top:13%}.s4{left:80%;top:28%}.s5{left:80%;top:72%}.s6{left:50%;top:87%}
      .fan-button{position:absolute;left:50%;top:50%;width:112px;height:112px;transform:translate(-50%,-50%);border:0;border-radius:50%;background:#faf9f7;display:grid;place-items:center;cursor:pointer;box-shadow:inset 0 2px 7px #0001,0 7px 16px #0001}.fan-button.on{background:#292a2b}.fan-icon{width:52px;height:52px}
      .reverse{display:block;margin:22px auto 0;min-width:180px;height:58px;border:0;border-radius:40px;background:#fbfaf8;color:#292929;box-shadow:0 8px 18px #0002,0 2px 5px #0001;font-size:17px;font-weight:700;cursor:pointer}
      .modes,.timer{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.modes{margin-top:38px}.mode,.timer button{min-height:65px;border:0;border-radius:35px;background:#fbfaf8;color:#292929;box-shadow:0 8px 18px #0002,0 2px 5px #0001;cursor:pointer}.mode{font-size:20px;font-weight:600}.timer-title{margin-top:38px}.timer button{font-size:19px;font-weight:700}
      .coming{padding:100px 25px;text-align:center;color:#777}.coming-icon{font-size:55px}.coming-title{font-size:22px;font-weight:700;letter-spacing:2px;margin-top:15px}.coming-text{margin-top:10px;font-size:14px}
...
class UniversalRemoteCardEditor extends HTMLElement {
  ...
  _defaultName(n,r){
    ...
    return n===1?'Basic Celling Fan':'Basic Celling Fan';
  }
  ...
  _schema(n){
    const p=`remote${n}_`,r=this._old(n),L=`Remote ${n}`,f=[];
  ...
  _collect(v){
    const room=n=>{
      ...
      return {
        name:this._multiple?(v[p+'name']||`REMOTE ${n}`):undefined,
        design,
        fan_name:v[p+'fan_name']||fallback,
        fan:v[p+'fan'],light:v[p+'light'],actions:a
      };
    };
    const rooms={remote1:room(1)};
    if(this._multiple)rooms.remote2=room(2);
    return {...this._config,multiple_remotes:this._multiple,rooms};
  }
...
UniversalRemoteCard.getStubConfig=()=>({multiple_remotes:false,rooms:{remote1:{design:'fan',fan_name:'Basic Celling Fan'}}});
