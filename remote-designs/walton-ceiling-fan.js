// Walton Ceiling Fan remote - compact layout with icons.
  ...
  render(ctx) {
    ...
    return `
      <style>
        ...
        .walton-circle{position:relative;width:min(430px,100%);aspect-ratio:1;margin:0 auto 16px;border-radius:50%;background:radial-gradient(circle,var(--c1),var(--c2));border:1px solid var(--border);box-shadow:inset 0 0 24px var(--border),0 15px 30px var(--shadow)}
        .walton-speed{position:absolute;width:62px;height:62px;border:0;border-radius:50%;background:var(--button);color:var(--button-text);font-size:21px;font-weight:650;box-shadow:0 8px 18px var(--shadow);cursor:pointer;transform:translate(-50%,-50%);transition:.1s}
